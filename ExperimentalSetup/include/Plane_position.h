...
class Plane_position
{
 public:
  ...
  void getPosition(float xI, float xJ, float* pos) { for (int i=0; i<3; ++i) pos[i]=position_origin[i]+xI*direction_I[i]+xJ*direction_J[i];}
  void getPosition(float xI, float xJ, std::array<float,3>& pos) { getPosition(xI,xJ,pos.data());}
 private:
