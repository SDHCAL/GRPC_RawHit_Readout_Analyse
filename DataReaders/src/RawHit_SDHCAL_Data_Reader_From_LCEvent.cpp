...
void RawHit_SDHCAL_Data_Reader_From_LCEvent::processEvent(EVENT::LCEvent * evt)
{
  ...
  if (NULL==col) std::cout << "ERROR collection " << m_collectionName << " not found" << std::endl;
...
{
  std::cout << "RawHit_SDHCAL_Data_Reader_From_LCEvent has processed " << m_nProcessedEvent << " readouts." << std::endl;
}
